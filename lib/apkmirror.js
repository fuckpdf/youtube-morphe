...
async function pageExists(page, url, allowRetry = true) {
  ...
  if (looksBlocked) {
    ...
    if (allowRetry) {
      console.log("   ↳ Waiting 8s for challenge to auto-resolve, then retrying...");
      await sleep(8000);
      return pageExists(page, url, false);
    }
    console.log("   ↳ Still blocked after retry, giving up on this URL");
  } else {
