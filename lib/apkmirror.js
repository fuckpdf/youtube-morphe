...
async function downloadApk(version, appName = "youtube") {
  ...
  try {
    ...
    const variantUrl = await page.evaluate(() => {
      ...
      // Kabul ettiğimiz güvenli mimari listesi (Python kodundaki _DEFAULT_ARCH mantığı)
      ...
      for (const row of rows) {
        ...
        // Python kodundaki gibi rozet (badge) kontrolü yaparak Bundle ayrımını kesinleştiriyoruz
        ...
        // Mimari kontrolü (Hücre boşsa veya izin verilen listemizdeyse geçerli say)
        const isTargetArch = archText === "" || allowedArchs.some(arch => archText.includes(arch));

        // DPI kontrolü (nodpi, anydpi veya dinamik çoklu çözünürlükleri kapsar)
      ...
      // Standalone APK önceliklidir, eğer yoksa Reddit için Bundle (PAKET) linkini döndürür
