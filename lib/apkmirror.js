...
async function downloadApk(version, appName = "youtube", forceBuild = null) {
  ...
  try {
    ...
    // appName'i de parametre olarak evaluate fonksiyonuna yolluyoruz
    const variantUrl = await page.evaluate(({ targetBuild, app }) => {
      ...
      for (const row of rows) {
        ...
        if (targetBuild && !cells[0].innerText.includes(targetBuild)) {
          continue;
        }
        ...
        // INSTAGRAM İÇİN ÖZEL KURAL: Sadece .apkm (Bundle) olanları kabul et
