  ...
  try {
    ...
    const variantUrl = await page.evaluate(() => {
      ...
      for (const row of rows) {
        ...
        const isTargetArch = arch.includes("universal") || arch.includes("noarch") || arch.includes("arm64-v8a");
        ...
        if (isTargetArch) {
          ...
          if (a) {
            if (dpi.includes("nodpi")) {
              nodpiVariant = a.href;
              break; // nodpi bulursak en idealidir, aramayı bitir.
            } else {
              anyDpiVariant = a.href; // nodpi yoksa yedek olarak bunu tut.
            }
          }
        }
      }
