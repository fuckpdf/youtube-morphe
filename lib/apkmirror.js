...
async function downloadApk(version, appName = "youtube") {
  ...
  try {
    ...
    // Kritik Düzenleme: URL sonuna ?apptype=apk ekleyerek bundle (paket) olan her şeyi siteden gizliyoruz.
    const listUrl = `https://www.apkmirror.com/apk/${appConfig.org}/${appConfig.slug}/${appConfig.slug}-${versionSlug}-release/?apptype=apk`;
    ...
    const variantUrl = await page.evaluate(() => {
      ...
      let nodpiVariant = null;
      let anyDpiVariant = null;
        ...
        if (isTargetArch) {
          ...
          if (a) {
            if (dpi.includes("nodpi")) {
              ...
              anyDpiVariant = a.href; 
            }
          }
        }
      }

      return nodpiVariant || anyDpiVariant;
    });

    if (!variantUrl) throw new Error("No standalone variant found on APKMirror");
