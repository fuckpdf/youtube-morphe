...
function patchApk(desktop, patches, apk, extraArgs = "", arch = "arm64-v8a") {
  ...
  try {
    ...
    if (output.includes("Applying 0 patches")) {
      throw new Error("Applying 0 patches. Uyumlu yama bulunamadı veya sürüm desteklenmiyor.");
    }
    ...
    if (stdout.includes("Applying 0 patches") || stderr.includes("Applying 0 patches")) {
      throw new Error("Applying 0 patches. Uyumlu yama bulunamadı veya sürüm desteklenmiyor.");
    }
