...
async function getOrCreateRelease(tag, releaseBody = "") {
  ...
  if (res.message === "Not Found") {
    ...
    console.log("♻️ Updating existing release body...");
    await request(
      {
        hostname: "api.github.com",
...
async function ensureRelease(tag, releaseBody) {
  assertConfigured();
  return getOrCreateRelease(tag, releaseBody);
}
...
async function uploadMicroGOnce(release) {
  ...
  const assets = await getAssets(release.id);
  const alreadyUploaded = assets.find(a => a.name === microgResult.name);
  ...
  await uploadWithReplace(release, microgResult.name);
}
