...
async function createNewRelease(tag, releaseName, releaseBody = "") {
  ...
  if (!res.id) {
    throw new Error(`Failed to create release: ${JSON.stringify(res)}`);
  }
...
async function ensureRelease(tag, releaseName, releaseBody) {
  assertConfigured();
  return createNewRelease(tag, releaseName, releaseBody);
}
