...
async function fetchLatestRelease(owner, repo, prerelease = false) {
  ...
  return withRetry(async () => {
    ...
    if (prerelease) {
      const found = data.find(r => r.prerelease === true);
      if (!found) throw new Error("No prerelease found");
      return found;
    }

    const found = data.find(r => r.prerelease === false && !r.draft);
    if (!found) throw new Error("No stable release found");
    return found;
  });
}
